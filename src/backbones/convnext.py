...
class ConvNeXtBackbone(BaseBackbone):
    """ConvNeXt backbone with multi-scale outputs.

    Uses timm's feature extraction to get intermediate features.
    ConvNeXt is a modernized ConvNet that matches transformer performance.
    """
    ...
    def __init__(self, cfg: dict[str, Any]) -> None:
        ...
        Args:
            cfg: Configuration with keys:
                - variant: 'convnext_tiny', 'convnext_small', 'convnext_base' (default: 'convnext_tiny')
                - pretrained: Whether to use pretrained weights (default: True)
        """
        super().__init__()

        variant = cfg.get("variant", "convnext_tiny")
        ...
        self.model = timm.create_model(
            ...
            features_only=True,
            out_indices=(0, 1, 2, 3),  # Get features at multiple scales
        )
...
# Register variants
BackboneFactory.register("convnext_tiny", ConvNeXtBackbone)
BackboneFactory.register("convnext_small", ConvNeXtBackbone)
BackboneFactory.register("convnext_base", ConvNeXtBackbone)
