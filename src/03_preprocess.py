...
# Default valid color classes for the VCR task.
# Used as a fallback if not provided in config.
DEFAULT_VALID_COLOR_CLASSES = {
    "white", "silver", "black", "gray", "red", "blue",
    "green", "yellow", "brown", "orange", "purple",
}
...
def validate_records(
    ...
    for record in records:
        ...
        # Check label is a valid color class (not a vehicle type like "car")
        if require_label:
            label_lower = record["label"].strip().lower()
            if label_lower in ("", "null", "none") or label_lower not in valid_classes:
                errors.append(f"{record_id}: Invalid color label '{record['label']}'")
                continue
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--split-from-dirs",
        action="store_true",
        help="Use pre-existing splits from manifest records (set by 01_detect_crop multi-source)",
    )
    parser.add_argument(
        "--seed",
        type=int,
...
class Step03Preprocess(PipelineStep):
    ...
    def __init__(self, config: dict[str, Any] | None = None) -> None:
        ...
        self.split_from_dirs: bool = False
        self.valid_classes: set[str] = DEFAULT_VALID_COLOR_CLASSES
    ...
    def run(self) -> int:
        ...
        split_from_dirs = self.split_from_dirs or preprocess_cfg.get("split_from_dirs", False)

        if split_from_dirs:
            # Use pre-existing splits from directory structure (set by 01_detect_crop)
            records_with_split = [r for r in valid_records if r.get("split")]
            records_without_split = [r for r in valid_records if not r.get("split")]

            if records_without_split:
                logger.warning(
                    f"{len(records_without_split)} records have no pre-assigned split. "
                    f"Assigning them to 'train'."
                )
                for r in records_without_split:
                    r["split"] = "train"

            logger.info(f"Using directory-based splits ({len(records_with_split)} pre-assigned, "
                        f"{len(records_without_split)} defaulted to train)")
        else:
        ...
        # Per-source split summary (if source_dataset is present)
        sources_present = set(
            r.get("meta", {}).get("source_dataset")
            for r in valid_records
            if r.get("meta", {}).get("source_dataset")
        )
        if sources_present:
            logger.info("Per-source split distribution:")
            for src in sorted(sources_present):
                src_records = [r for r in valid_records if r.get("meta", {}).get("source_dataset") == src]
                src_splits = Counter(r["split"] for r in src_records)
                logger.info(f"  {src}: {dict(src_splits)}")
...
def main() -> int:
    ...
    step.split_from_dirs = args.split_from_dirs
    
    # Extract valid classes from config if available
    preprocess_cfg = cfg.get("preprocess", {})
    if "valid_classes" in preprocess_cfg:
        step.valid_classes = set(preprocess_cfg["valid_classes"])
        logger.info(f"Using {len(step.valid_classes)} valid classes from config")
    else:
        logger.info(f"Using default {len(DEFAULT_VALID_COLOR_CLASSES)} valid classes")
