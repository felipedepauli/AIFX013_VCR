...
def load_model(checkpoint_path: Path, device: torch.device) -> tuple[VCRModel, dict[str, Any]]:
    ...
    # Also handle legacy flat config and optimization nested config ("train")
...
def predict_batch(
    ...
    for i, (pred_idx, probs) in enumerate(zip(all_preds, all_probs)):
        ...
        # Include source dataset for per-source evaluation
        source = record.get("meta", {}).get("source_dataset")
        if source:
